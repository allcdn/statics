...
function PlayHistoryClass() {
	...
	this.viewPlayHistory = function(div) {
		...
		if(checkcookie()){
			$.get(Root +"index.php?s=/home/playlog/ajax", function(result){												 
				if(result['code']>-1){
					$("#his-todo").hide();
	...
	this.removeHistory = function(ii, type) {
		...
		if (type == 'inck') {
			...
			$.get(Root + 'index.php?s=/home/playlog/del',{log_id:ii},function(r){},'json');
		}
	}
	this.addPlayHistory = function(json,vod_readurl,vod_palyurl) {
		var count = 10; // 播放历史列表调用条数
		if(checkcookie()){
			$.post(Root + "index.php?s=/home/playlog/add",json);
		}
	...
	this.emptyhistory = function(type){
		$.showfloatdiv({
			...
			suredo : function(e) {
				if(type=='ajax'){
					$.get(Root + 'index.php?s=/home/playlog/clear',function(r){document.getElementById('playhistory').innerHTML="<li class='no-his'><p>暂无播放历史列表...</p></li>";});
				}else{
					_GC();
				}
				$.closefloatdiv();

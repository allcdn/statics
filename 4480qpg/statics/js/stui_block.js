...
if (window.console && window.console.log) {
	console.log('主题名称：青苹果影院');
	console.log('主题官网：https://www.4480qpg.net');
	...
	'images': {//图片处理
		...
		'qrcode': function () {
			if (qrcode1 == 1) {
				...
				if ($("#qrcode").length) {
					$("#qrcode").append("<img class='img-responsive' src='" + qrcode1 + "' width='100%' height='100%' />")
				}
			}
			if (qrcode2 == 1) {
				if ($("#qrcode2").length) {
				...
				if ($("#qrcode2").length) {
					$("#qrcode2").append("<img class='img-responsive' src='" + qrcode2 + "' width='100%' height='100%' />")
				}
			}
		}
	},
